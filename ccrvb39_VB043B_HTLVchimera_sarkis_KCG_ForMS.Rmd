...
params:
  ...
  input_dir: "~/../../Volumes/NCI_VB/franchini/ccrvb39_VB043B_HTLVchimera_sarkis_KCG/trimmedForMS_Sept2024/input_data" ### includes fixed typo in DHF6 as of 6/12/26
...
##### first format the processed the (blood only) FACS data for batch 1
### for DC tabs
DC_df <- data.frame()
...
phen_long <- list_of_datasets[["Virus_variables"]] %>%
  ...
  ### add DC batch2
   dplyr::full_join(list_of_datasets[["DC_blood_batch2"]] %>%
                     pivot_longer(cols = !c(!!!rlang::syms(meta_vars_short)), names_to = "variable", values_to = "amount"),
                   by = c(meta_vars_short,"variable","amount")) %>%
    ### add monocytes
  dplyr::full_join(monocytes_df %>%
   ...
   dplyr::full_join(list_of_datasets[["Monocytes_blood_batch2"]] %>%
                     pivot_longer(cols = !c(!!!rlang::syms(meta_vars_short)), names_to = "variable", values_to = "amount"),
                   by = c(meta_vars_short,"variable","amount")) %>%
  ...
  dplyr::full_join(neutrophil_df %>%
   ...
   dplyr::full_join(list_of_datasets[["Neutrophil_blood_batch2"]] %>%
                     pivot_longer(cols = !c(!!!rlang::syms(meta_vars_short)), names_to = "variable", values_to = "amount"),
                   by = c(meta_vars_short,"variable","amount")) %>%
  
  ####### add new BAL gating
   dplyr::full_join(list_of_datasets[["DCMonoNeut_BAL"]] %>%
                     pivot_longer(cols = !c(!!!rlang::syms(meta_vars_short)), names_to = "variable", values_to = "amount") %>%
...
### Fig panel 
name_to_use <- paste0("Fig2c",shortName_for_wtBatch)
levels_or_FC <- "levels"
...
<!--     ### save values plotted -->
<!-- list_of_plotted[[paste0("Fig2c",shortName_for_wtBatch)]] <- phen_to_plot[,c("Animal_ID","Treatment","Virus",rownames(p_value_df2)[which(p_value_df2$TriMutvsTriWT_p_raw < 0.05)])] -->
...
```{r Fig2d_viralscoreHeatmap, results='asis', fig.width=12, fig.height=10, eval = eval_Fig2}
...
### save combo to add to data
  ...
  list_of_plotted[[paste0("Fig2de_prep",shortName_for_wtBatch)]] <-  rbind(vs_scores_prep1,vs_scores_prep2[,colnames(vs_scores_prep1)]) %>%
    rownames_to_column("variable_time")
  
    list_of_plotted[[paste0("Fig2d",shortName_for_wtBatch)]] <-  viral_score_df %>%
    rownames_to_column("variable_time")
```
...
name_prefix <- "Fig2e"
long_name_to_use <- "BothSumSc"
 name_to_use <- paste0("Fig2de_",str_remove_all(long_name_to_use, " "),shortName_for_wtBatch)
p_value_df1 <- list_of_stats[[paste0(name_to_use,"_",levels_or_FC)]] %>%
  column_to_rownames("variable_time")
...
#  }
      ...
      ### save values plotted
list_of_plotted[[paste0("Fig2e",shortName_for_wtBatch)]] <- phen_to_plot[,c("Animal_ID","Treatment","Virus","COMBO")]
...
fig_name <- paste0("SFig2a4",shortName_for_wtBatch)
...
list_of_stats[[paste0(fig_to_use,"_pvalsPlotted")]] <- pval_combined %>%
...
```{r SFig4_prep}
fig_to_use <- paste0("SFigS4",shortName_for_wtBatch)
###### select group/tp from CWT analysis
### treatment virus comp
...
``` {r setVars___DCs}
fig_name <- paste0("Fig3a",shortName_for_wtBatch)
...
"),"\n")
  ...
  list_of_plotted[[paste0(fig_name,"_",comp,"",shortName_for_wtBatch)]] <- to_save
...
``` {r levels_ScatterplotWithBox, results = 'asis', fig.height = 11, fig.width = 8}
...
fig_name <- paste0("Fig3b",shortName_for_wtBatch)
...
curr_vars_prep <- unique(curr_phen_long$variable)
curr_vars_for_plot <- c("Pct_Monocytes","Pct_Classical","Pct_Intermediate","Pct_Nonclassical",
                        sort(curr_vars_prep[grepl("_TNFa",curr_vars_prep)]),
                        sort(curr_vars_prep[grepl("_IL8",curr_vars_prep)]),
                        sort(curr_vars_prep[grepl("_IL10",curr_vars_prep)]))
limits_to_use <- c(-10,100)
breaks_to_use <- c(0,25,50,75,100)
labels_to_use <- c(0,25,50,75,100)
y.pos_to_use <- limits_to_use[1]
...
``` {r levels_ScatterplotWithBox___BAL___Mono, ref.label=c('levels_ScatterplotWithBox'), results = 'asis', fig.height = 14, fig.width = 8}
```
...
fig_name <- paste0("Fig3c",shortName_for_wtBatch)
...
list_of_plotted[[paste0("SFigS6",shortName_for_wtBatch)]] <- paste0("see Fig3a, Fig3b, Fig3c tabs ",shortName_for_wtBatch)
list_of_plotted[[paste0("SFigS7",shortName_for_wtBatch)]] <- paste0("see Fig3a, Fig3b, Fig3c tabs ",shortName_for_wtBatch)

list_of_stats[[paste0("SFigS6",shortName_for_wtBatch)]] <- paste0("see Fig3a, Fig3b, Fig3c tabs ",shortName_for_wtBatch)
list_of_stats[[paste0("SFigS7",shortName_for_wtBatch)]] <- paste0("see Fig3a, Fig3b, Fig3c tabs ",shortName_for_wtBatch)
...
fig_name <- paste0("Fig4abc",shortName_for_wtBatch)
...
```{r addSuppFigS8}
list_of_plotted[[paste0("SFigS8",shortName_for_wtBatch)]] <- paste0("see Fig4abc tabs ",shortName_for_wtBatch)

list_of_stats[[paste0("SFigS8",shortName_for_wtBatch)]] <- paste0("see Fig4abc tabs ",shortName_for_wtBatch)
```
...
fig_name <- paste0("SFig9_cytvNoD",shortName_for_wtBatch)
vars_to_use <- cytokine_columns
var_name <- "cytokines"
...
## Associations between sig different cytokines & cell subsets (Fig 4def)  {.tabset}
...
contrast_name <- paste0("Fig4def",shortName_for_wtBatch)
...
sig_diff_cytokines <- list_of_stats[[paste0("Fig4abc",shortName_for_wtBatch,"_MW_",compartment_var2,"_",levels_or_BOTH2)]] %>%
  ...
  for(curr_ds in names(list_of_trimmed_dfs)){
    ...
    for (tp in timepoints_with_both){
      ...
      openxlsx::write.xlsx(list_of_corr, file = file.path(outputTABLES_dir,
                                                          paste0(params$study_name,"_Corr",var_name1,"Vs",var_name2,"",shortName_for_wtBatch,"_",tp,"_",group_name,"_",curr_ds,"_",method_to_use,"_CoefPvals.xlsx")),
                           rowNames=T)

      ### also save as rdata to be able to access quickly
      save(cor_mat, cor_test_mat, cor_test_adj_mat, file = file.path(outputRData_dir,
                                                                     paste0(params$study_name,"_Corr",var_name1,"Vs",var_name2,"",shortName_for_wtBatch,"_",tp,"_",group_name,"_",curr_ds,"_",method_to_use,"_CoefPvals.RData")))
    ...
    openxlsx::write.xlsx(list_of_corr, file = file.path(outputTABLES_dir, 
                                                        paste0(params$study_name,"_Corr",var_name1,"Vs",var_name2,"",shortName_for_wtBatch,"_comboTP_",group_name,"_",curr_ds,"_",method_to_use,"_CoefPvals.xlsx")),
                         rowNames=T)
    
    ### also save as rdata to be able to access quickly
    save(cor_mat_combo, cor_test_mat_combo, num_animals_df, file = file.path(outputRData_dir, 
                                                                             paste0(params$study_name,"_Corr_",var_name1,"Vs",var_name2,"",shortName_for_wtBatch,"_comboTP_",group_name,"_",curr_ds,"_",method_to_use,"_CoefPvals.RData")))
    
    saveRDS(timepoints_with_both, file = file.path(outputRData_dir, 
                                                                             paste0(params$study_name,"_Corr_",var_name1,"Vs",var_name2,"",shortName_for_wtBatch,"_numTPsBoth_",group_name,"_",curr_ds,"_",method_to_use,"_CoefPvals.rds")))
...
for(i in 1:length(group_list)){
  ...
  curr_ds <- "levels"
    ...
    for(tp in timepoints_with_both){
      ...
      Corr_Name <- paste0(params$study_name,"_Corr",var_name1,"Vs",var_name2,"",shortName_for_wtBatch,"_",tp,"_",group_name,"_",curr_ds)
      Corr_Name_combo <- paste0(params$study_name,"_Corr_",var_name1,"Vs",var_name2,"",shortName_for_wtBatch,"_comboTP_",group_name,"_",curr_ds)
...
###### save stats
        list_of_stats[[paste0(fig_name_to_use,"_MW_Lung",short_name_to_use)]] <- curr_p_df
...
## gaps for rows
 ...
 cat("\n")
  cat(' \n \n') ### this is the key!
    ...
    list_of_plotted[[paste0(fig_name_to_use,"_",short_name_to_use)]] <- df_for_plot %>%
      rownames_to_column("variable")
