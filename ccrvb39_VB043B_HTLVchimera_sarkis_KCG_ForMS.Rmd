...
### Supp Fig X Alveolar Macrophages heatmap {.tabset}

``` {r setVars___Macrophages}
fig_name <- paste0("SuppFigX_",shortName_for_wtBatch)
vars_to_use <- unique(phen_long$variable)[grepl("Macrophages",unique(phen_long$variable))]
var_name <- "Macrophages"
treatment_to_use <- "Triple_depletion"
...
```{r MW_3Group_levels_calculate___Macrophages, ref.label=c('MW_3Group_levels_calculate'), results='asis', fig.height=12, fig.width=7}
```


``` {r CellSubsets_levels_heatmap___Macrophages, ref.label=c('CellSubsets_levels_heatmap'), results = 'asis', fig.height = 6, fig.width = 8}
```

#### Scatterplots  {.tabset}

```{r scat_setup_bulkPops___Macrophages}
curr_vars_for_plot <- vars_to_use
limits_to_use <- c(-10,100)
breaks_to_use <- c(0,25,50,75,100)
labels_to_use <- c(0,25,50,75,100)
y.pos_to_use <- limits_to_use[1]
...
``` {r CellSubsets_levels_Scatterplot___Macrophages, ref.label=c('CellSubsets_levels_Scatterplot'), results = 'asis', fig.height = 12, fig.width = 7}
```
...
```{r scatBox_setup___Mac}

#### set order for only those that are sig diff
curr_vars_for_plot_df <- list_of_stats[[paste0(fig_name,"_MW_BAL_",levels_or_FC)]] %>%
          dplyr::mutate(var_time_comp = paste0(variable_time,"__BAL")) %>%
          dplyr::select(!!!rlang::syms(colnames(.))) %>%
  ### filter to only sig by Chi vs WT
  dplyr::filter(grepl("p < 0.05", TriMutvsTriWT_dir))
...
```{r setBAL___Mac, ref.label=c('setBAL')}
```

``` {r levels_ScatterplotWithBox___BAL___Mac, ref.label=c('levels_ScatterplotWithBox'), results = 'asis', fig.height = 6, fig.width = 8}
```
...
vars_to_use1 <- c(DC_columns,monocyte_columns,neutrophil_columns,unique(phen_long$variable)[grepl("Macrophages",unique(phen_long$variable))])
